...
data class Time(
    ...
    companion object Companion {
        // Parse from 10:14:00+02
        fun parseFromTimeTz(string: String): Time {
        ...
        fun parseFromTimestampTz(timestampTz: String): Time {
            // A timestamp with time zone is stored in the "yyyy-MM-dd HH:mm:ss.SSSSSS+HH" format
    ...
    fun parseToTimestampTz(date: LocalDate): String {
        ...
        Log.i("Time", "dateString: $dateString")
...
//            .format(Instant.now())
        ...
        return timestamp + OffsetDateTime.now().offset
    }

    fun parseToTimeTz(): String {
        // A time with time zone is stored in the 10:14:00+02 format
        val hour = this.hour.toString().padStart(2, '0')
        val minute = this.minute.toString().padStart(2, '0')
        val second = "00"
        val timeString = arrayOf(hour, minute, second).joinToString(":")

        return (timeString + OffsetDateTime.now().offset).dropLast(3)
    }
