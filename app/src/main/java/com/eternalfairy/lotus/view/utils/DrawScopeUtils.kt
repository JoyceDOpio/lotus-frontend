package com.eternalfairy.lotus.view.utils
...
import com.eternalfairy.lotus.view.theme.HotPink
...
object DrawScopeUtils {
    ...
    fun DrawScope.drawClockHand(
        ...
        val startOffset = Offset(
            x = center.x + (startRadius * cos(angle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
            y = center.y + (startRadius * sin(angle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
        )
        val endOffset = Offset(
            x = center.x + (endRadius * cos(angle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
            y = center.y + (endRadius * sin(angle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
        )
        val path = Path()
    ...
    fun DrawScope.drawHourStepsAndLabels(
        ...
        for (i in 0..(minutesBetweenHoursAccumulated.size - 2)) {
            // Draw hour steps
            var stepAngle = minutesBetweenHoursAccumulated[i] * minuteAngle + TouchGestureUtils.DEG_OFFSET
            ...
            stepAngle = minutesBetweenHoursAccumulated[i] * minuteAngle + TouchGestureUtils.DEG_OFFSET
            var stepLabelOffset = Offset(
                x = center.x + (outerRadius * cos(stepAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
                y = center.y + (outerRadius * sin(stepAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
            )
    ...
    fun DrawScope.drawMinuteSteps(
        ...
        for (i in 0..totalMinutes) {
            if (i % minuteStep == 0 && !(i in minutesBetweenHoursAccumulated)) {
                val stepAngle = i * minuteAngle + TouchGestureUtils.DEG_OFFSET
    ...
    fun DrawScope.drawTask(
        ...
        val gradient = Brush.Companion.sweepGradient(
        ...
        var borderStartOffset = Offset(
            x = center.x + cos(taskStartAngle * TouchGestureUtils.DEG_TO_RAD).toFloat(),
            y = center.y + sin(taskStartAngle * TouchGestureUtils.DEG_TO_RAD).toFloat()
        )
        var borderEndOffset = Offset(
            x = center.x + (outerRadius * cos(taskStartAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
            y = center.y + (outerRadius * sin(taskStartAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
        )
        ...
        borderStartOffset = Offset(
            x = center.x + cos(taskEndAngle * TouchGestureUtils.DEG_TO_RAD).toFloat(),
            y = center.y + sin(taskEndAngle * TouchGestureUtils.DEG_TO_RAD).toFloat()
        )
        borderEndOffset = Offset(
            x = center.x + (outerRadius * cos(taskEndAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
            y = center.y + (outerRadius * sin(taskEndAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
        )
        ...
        if (taskTitle != null && taskDurationInMinutes != null && textMeasurer != null && canvasHeight != null && canvasWidth != null && minuteAngle != null && innerRadius != null) {
            ...
            // Define radius path
                ...
                val titleStartOffset = Offset(
                    x = center.x + (startRadius * cos(middleAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
                    y = center.y + (startRadius * sin(middleAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
                )
                val titleEndOffset = Offset(
                    x = center.x + (endRadius * cos(middleAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat(),
                    y = center.y + (endRadius * sin(middleAngle * TouchGestureUtils.DEG_TO_RAD)).toFloat()
                )
