...
import com.example.circularplanner.ui.component.CLOCK_CENTER_COLOR
import com.example.circularplanner.ui.component.CLOCK_LABEL_COLOR
import com.example.circularplanner.ui.component.HOUR_LABEL_COLOR
import com.example.circularplanner.ui.component.MINUTE_STEP_COLOR
    ...
    fun DrawScope.drawTask(
        ...
        if (
            ...
            // Define radius path
                ...
                val taskEndAngleAngleTranslated = TouchGestureUtils.translateAngle270To0(taskEndAngle)
                val taskStartAngleTranslated = TouchGestureUtils.translateAngle270To0(taskStartAngle)
                val startArc = (startRadius * (taskEndAngleAngleTranslated - taskStartAngleTranslated) * Math.PI / 180) + pathPadding
                val endArc = (endRadius * (taskEndAngleAngleTranslated - taskStartAngleTranslated) * Math.PI / 180) + pathPadding

                if (startArc <= titleHeight || endArc <= titleHeight) drawTitle = false
                Log.i("TaskDial", "startArc $startArc")
                Log.i("TaskDial", "endArc $endArc")
                Log.i("TaskDial", "titleHeight $titleHeight")
